 ...
 * @author Lukas Eder
 */
public final class StringBinding<T, U> implements Binding<T, U> {

    static final JooqLogger     log              = JooqLogger.getLogger(StringBinding.class);
    ...
    public StringBinding(Converter<T, U> converter) {
        this(converter, false);
    }

    public StringBinding(Converter<T, U> converter, boolean isLob) {
    ...
    private final void toSQL(BindingSQLContext<U> ctx, Object val) {
        ...
        if (render.paramType() == INLINED) {
            ...
            // escape syntax
                ...
                if (family == H2) {
                    ...
                    for (Object o : ((Object[]) val)) {
                        ...
                        new StringBinding<Object, Object>(new IdentityConverter(type.getComponentType()), isLob).sql(new DefaultBindingSQLContext<Object>(ctx.configuration(), ctx.render(), o));
                        separator = ", ";
                    }
            ...
            xx [/pro] */
                ...
                if (literal == null) {
                    new StringBinding<Object, Object>(new IdentityConverter(String.class), isLob).sql(new DefaultBindingSQLContext<Object>(ctx.configuration(), ctx.render(), literal));
                }
                else {
                    new StringBinding<Object, Object>(new IdentityConverter(String.class), isLob).sql(new DefaultBindingSQLContext<Object>(ctx.configuration(), ctx.render(), literal));
                }
...
//        }
    ...
    @Override
    public void get(BindingGetResultSetContext<U> ctx) throws SQLException {
        T result = (T) ctx.resultSet().getString(ctx.index());
    ...
    @Override
    public void get(BindingGetStatementContext<U> ctx) throws SQLException {
        T result = (T) ctx.statement().getString(ctx.index());
        ctx.value(converter.from(result));
    }
        ...
        // This might be a UDT (not implemented exception...)
            ...
            try {
                while (rs.next()) {
                    ...
                    new StringBinding<T, T>(new IdentityConverter<T>((Class<T>) type.getComponentType()), false).get(out);
