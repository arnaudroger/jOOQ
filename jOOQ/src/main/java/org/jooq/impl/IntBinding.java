 ...
 * @author Lukas Eder
 */
public final class IntBinding<T, U> implements Binding<T, U> {

    static final JooqLogger     log              = JooqLogger.getLogger(IntBinding.class);
    ...
    public IntBinding(Converter<T, U> converter) {
        this(converter, false);
    }

    public IntBinding(Converter<T, U> converter, boolean isLob) {
    ...
    private final void toSQL(BindingSQLContext<U> ctx, Object val) {
        ...
        if (render.paramType() == INLINED) {
            ...
            // escape syntax
                ...
                if (family == H2) {
                    ...
                    for (Object o : ((Object[]) val)) {
                        ...
                        new IntBinding<Object, Object>(new IdentityConverter(type.getComponentType()), isLob).sql(new DefaultBindingSQLContext<Object>(ctx.configuration(), ctx.render(), o));
                        separator = ", ";
                    }
            ...
            xx [/pro] */
                ...
                if (literal == null) {
                    new IntBinding<Object, Object>(new IdentityConverter(String.class), isLob).sql(new DefaultBindingSQLContext<Object>(ctx.configuration(), ctx.render(), literal));
                }
                else {
                    new IntBinding<Object, Object>(new IdentityConverter(String.class), isLob).sql(new DefaultBindingSQLContext<Object>(ctx.configuration(), ctx.render(), literal));
                }
...
//        }
    ...
    @Override
    public void get(BindingGetResultSetContext<U> ctx) throws SQLException {
        T result = (T) Integer.valueOf(ctx.resultSet().getInt(ctx.index()));
    ...
    @Override
    public void get(BindingGetStatementContext<U> ctx) throws SQLException {
        T result = (T) Integer.valueOf(ctx.statement().getInt(ctx.index()));
        ctx.value(converter.from(result));
    }
    ...
    @Override
    public U get(Configuration configuration, ResultSet resultSet, int i) throws SQLException {
        return converter.from((T) Integer.valueOf(resultSet.getInt(i)));
    }
        ...
        // This might be a UDT (not implemented exception...)
            ...
            try {
                while (rs.next()) {
                    ...
                    new IntBinding<T, T>(new IdentityConverter<T>((Class<T>) type.getComponentType()), false).get(out);
